...
1. HTTP Method : 데이터를 다루는 방법에 대한 내용을 담는다. 일종의 동사라고 생각할 수 있다. HTTP Method의 대표적인 종류는 다음과 같다.
     ...
     * **PUT** : 데이터를 생성해달라고 요청한다. 단, 중복 생성을 허용하지 않는다. 만약 중복된 데이터가 존재하면 그 데이터는 지우고 새롭게 PUT으로 요구한 데이터로 교체한다.
...
2. URL : 다룰 데이터의 위치를 의미한다. 일종의 명사다. 그래서 HTTP Method에 대한 대상을 URL에서 다룬다. URL은 다음과 같이 구성된다.
     ...
     * Path Paremeter : URL의 일반화된 표현방법.   
     ...
     가령 네이버에서 "ez1hub"를 검색한다고 하자. 그러면 ex1hub라는 매개값을 어떻게 서버 컴퓨터에서 처리할 것인가? 서버 컴퓨터에서 임의로 경로를 설정해 모든 데이터를 처리하기는 곤란하다. 이 경우에서는 **Query String**이라는 것을 통해 처리할 수 있다.
...
그래서 이를 방지하고자 획일화된 우편 발신 방법을 정한다. 정해진 자리에 보낸이의 이름과 주소를 적고, 받는이의 이름과 주소를 적고, 정해진 자리에 알맞은 크기의 우표를 붙이도록 하는 것이다.    
이를 API의 상황에 대입해보자. 이것이 웹 HTTP 상에서는 대표적으로 REST 아키텍쳐를 따르도록 설계한 **REST API**로 표현된다. REST API는 HTTP 요청 메세지와 마찬가지로 URL, HTTP Method로 구성되어 있다.
